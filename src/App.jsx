...
import Arte from './components/ResumeFile';
...
import Doom from './components/DoomApp.jsx';

import arte2 from './components/Arte2'
...
function App() {
  ...
  const [ArteExpand, setArteExpand] = useState(
  {expand: false, show: false, hide: false, focusItem: true, x: 0, y: 0, zIndex: 1,});

  const [Arte2Expand, setArte2Expand] = useState(
  {expand: false,hide: true,focusItem: false,show: true});
  ...
  const deleteIcon = ['Cat', 'AiAgent','3dObject','About','Mail'];
...
const handleOnDrag = (name, ref, type) => () => {
  ...
  if (iconRef && ResumeFolderRef.current && ProjectFolderRef.current) {
    ...
    if(name === 'MyComputer' || name === 'Lixeira') return; // prevent MyComputer from being dragged into folder
    ...
    if (
      ...
      if(name === 'Lixeira') return;
      setDropTargetFolder('Lixeira');
    }
    ...
    // Check for intersection with the Disk 
      ...
      const validFolders = ['DiskC', 'DiskD', 'Resume', 'Project', 'Picture', 'Lixeira', 'Utility', ...UserCreatedFolder.map(item => item.name)];
...
function handleDrop(e, name, target, oldFolderID) {
  ...
  await axios.post("https://notebackend-7heh.onrender.com/chat/send", {
    name: payload.name,
    chat: payload.chat
  });

  getChat(); // atualiza mensagens
...
async function getChat() {
  setChatData('')
  try {
    const response = await axios.get(`https://notebackend-7heh.onrender.com/chat/getchat`, {
      headers: {
...
function ObjectState() {
  return [
    ...
    { name: 'Arte',  setter: setArteExpand, usestate: ArteExpand, color: 'rgba(133, 165, 67, 0.85)', size: 'small' },
    { name: 'Doom',        setter: setDoomExpand,       usestate: DoomExpand,       color: 'rgba(53, 53, 53, 0.85)', size: 'small'   },
    ...
    { name: 'Lixeira',  setter: setBinExpand,        usestate: BinExpand,        color: 'rgba(64, 135, 66, 0.85)', size: 'small' },
...
function handleShow(name) {
  ...
  const notToOpenList = ['Run', 'Nft', 'Note', 'AiAgent', '3dObject', 'Fortune', 'PixelPic','IE'];
...
function handleShowMobile(name) {
  ...
  if (now - lastTapTime < 300) {
    ...
    const notToOpenList = ['Run', 'Note', 'AiAgent', '3dObject', 'Fortune', 'PixelPic','IE'];
    if (notToOpenList.includes(name)) return;
...
if (name === "Doom") {
    ...
    const dosboxDiv = document.getElementById("dosbox");

    // remove o canvas do emulador
    if (dosboxCanvas) {
      dosboxCanvas.remove();
    }
